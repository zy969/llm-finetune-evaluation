...
def analyze_metric(df_task, metric):
    ...
    if pivot_df.shape[1] < 2:
        print(f"[SKIP] {metric}: fewer than 2 models present.")
        return
    ...
    for model in pivot_df.columns:
        ...
        if len(data) < 3:
            normality[model] = 0
            print(f"[NOTE] {model} has too few samples for Shapiro test.")
        else:
            _, p = shapiro(data)
            normality[model] = p
            print(f"[Shapiro] {model}: p = {p:.4f}")

    all_normal = all(p > 0.05 for p in normality.values())

    # Statistical test
    if all_normal:
        print("[INFO] All groups pass normality. Using repeated-measures ANOVA + Tukey HSD.")
        ...
        print("[INFO] At least one group failed normality. Using Friedman + Wilcoxon with Holm correction.")
        ...
        reject, pvals_corr, _, _ = multipletests(p_values, method='holm')
    ...
    for i in range(len(pivot_df.columns)):
        for j in range(i + 1, len(pivot_df.columns)):
            ...
            x = pivot_df[m1].dropna()
            y = pivot_df[m2].dropna()
            x, y = x.align(y, join='inner')
            if len(x) < 2:
                print(f"[SKIP] {m1} vs {m2}: too few samples.")
                continue
